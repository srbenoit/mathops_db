...
/**
 * An immutable container for updated values for an update operation.  This class distinguishes between "do not update a
 * given field" and "update the field to NULL".
 *
 * <p>
 * When performing updates, applications need to supply new values for a subset of the fields in a table, but the
 * {@link Row} class is unsuitable for this task since every row must have values that match its constraints for every
 * field.
 *
 * <p>
...
public final class UpdatedValues {
     ...
     * @param theFieldValues the field values, indexed as in {@code theTable}, where {@code null} indicates the field is
     *                       not to be updated; a {@code NullValue} object indicates the field is to be updated to NULL,
     *                       and where trailing null values (non-updated fields) may be omitted
     * @throws IllegalArgumentException if the table name is null, there are more field values provided than the table
